...
void preferences_show_init() {
    ...
    g_signal_connect(window, "destroy", G_CALLBACK(gtk_widget_hide_all), window);
    ...
    g_signal_connect(run_state, "clicked", G_CALLBACK(c_start_work), NULL);
    g_signal_connect(stop, "clicked", G_CALLBACK(c_take_brake), NULL);
