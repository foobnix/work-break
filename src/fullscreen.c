...
static GdkPixbuf * get_screenshot() {
	...
	screenshot = gdk_pixbuf_get_from_drawable(NULL, root_window, NULL, x_orig,
...
//                pixel[i * rowstride + j + 2] = 255 - pixel[i * rowstride + j + 2];
				...
				avg += pixel[i * rowstride + j + 0]
						+ pixel[i * rowstride + j + 1]
						+ pixel[i * rowstride + j + 2];
				avg /= 3.00;
				avg = ((int) (avg)) % 256;
...
void fullscreen_show_init() {
	...
	gtk_label_set_markup(GTK_LABEL(text),
			"<span  font_desc='32.5' weight='bold' foreground='#663399'>Take a break!!!</span>");
	...
	g_signal_connect(GTK_OBJECT(f_window), "key_press_event",
			GTK_SIGNAL_FUNC(key_press), G_OBJECT(f_window));
...
void f_set_time_label(char *str) {
	gchar *res =
			g_strdup_printf(
					"<span  font_desc='32.5' weight='bold' foreground='#663399'>%s</span>",
					str);
